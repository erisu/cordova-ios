...
function overwritePrivacyManifest (manifest, locations) {
    const privacyManifestDest = path.join(locations.appDir, 'PrivacyInfo.xcprivacy');
    if (manifest != null) {
    ...
    const defaultPrivacyManifest = path.join(__dirname, '..', 'templates', 'project', APP_NAME, 'PrivacyInfo.xcprivacy');
...
function updateConfigFile (sourceConfig, configMunger, locations) {
    ...
    fs.cpSync(path.join(locations.cordovaDir, 'defaults.xml'), locations.configXml);
...
function updateProject (platformConfig, locations) {
    ...
    const plistFile = path.join(locations.appDir, 'App-Info.plist');
    ...
    return handleBuildSettings(platformConfig, locations);
}
...
function handleBuildSettings (platformConfig, locations) {
    ...
    if (deploymentTarget && fs.existsSync(podPath)) {
        ...
        const podfileFile = new Podfile(podPath, APP_NAME, deploymentTarget);
        podfileFile.write();
...
function updateIcons (cordovaProject, locations) {
    ...
    const platformProjDir = path.relative(cordovaProject.root, locations.appDir);
...
function cleanIcons (projectRoot, projectConfig, locations) {
    ...
    const platformProjDir = path.relative(projectRoot, locations.appDir);
    const iconsDir = path.join(platformProjDir, 'Assets.xcassets', 'AppIcon.appiconset');
