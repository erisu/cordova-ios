...
function startSim (appPath, target) {
    ...
    const workerPath = path.resolve(__dirname, './runSimulator.js');

    return new Promise((resolve, reject) => {
        const worker = new Worker(workerPath, { workerData: { appPath, deviceTypeId, logPath } });

        worker.on('message', function (data) {
            events.emit('log', `[simctl] ${data}`);
            // resolve(data);
        });
        worker.on('error', err => {
            events.emit('error', `[simctl] ${err.message}`);
            reject(err);
        });
        worker.on('exit', code => {
            if (code !== 0) {
