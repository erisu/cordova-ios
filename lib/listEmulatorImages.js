...
function fixNameKey (array = [], mapping = {}) {
    return array.map(elem => ({
        ...elem,
        name: mapping[elem.name] ?? elem.name
    }));
}

/**
 * Fixes the simctl list by replacing obfuscated with something more readable.
 * - Xcode 9 have obfuscated names for 2017 iPhones and Apple Watches.
...
function fixSimCtlList (list) {
    ...
    Object.entries(list.devices).forEach(([key, devices]) => {
        list.devices[key] = fixNameKey(devices, deviceNameMap);
    });
...
async function fetchSimCtlList () {
    const [
        { json: { devicetypes } },
        { json: { devices } },
        { json: { runtimes } }
    ] = await Promise.all([
        simctl.list({ devicetypes: true }),
        simctl.list({ devices: true }),
        simctl.list({ runtimes: true })
    ]);

    return fixSimCtlList({ devicetypes, devices, runtimes });
}

module.exports.fetchSimCtlList = fetchSimCtlList;
