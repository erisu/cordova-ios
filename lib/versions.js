...
/**
 * Gets simctl util version
 * @return {Promise} Promise that either resolved with simctl version
 *                           or rejected in case of error
 */
exports.get_simctl_version = async () => await simctl.simctl_version();

/**
 * Gets specific tool version
 * @param  {String} toolName Tool name to check. Known tools are 'xcodebuild', 'simctl' and 'ios-deploy'
...
exports.get_tool_version = toolName => {
    ...
    case 'simctl': return exports.get_simctl_version();
    case 'ios-deploy': return exports.get_ios_deploy_version();
    case 'pod': return exports.get_cocoapods_version();
    default: return Promise.reject(new CordovaError(`${toolName} is not valid tool name. Valid names are: 'xcodebuild', 'simctl', 'ios-deploy', and 'pod'`));
    }
};
