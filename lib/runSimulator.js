...
function launch (workerData) {
    const { appPath, deviceTypeId, logPath } = workerData;
    ...
    bplist.parseFile(infoPlistPath, function (err, obj) {
        ...
        parentPort.postMessage(`device.name: ${device.name}`);
        parentPort.postMessage(`device.runtime: ${device.runtime}`);
        parentPort.postMessage(`device.id: ${device.id}`);
        ...
        if (logPath) {
            console.log(`logPath: ${path.resolve(logPath)}`);
        }
